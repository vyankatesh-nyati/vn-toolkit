...
Measure before deciding: `wc -l <path>`. A URL has no line count until it is retrieved — fetch it
first, then measure the retrieved text the same way.
...
That is the highest existing ID — add one and zero-pad to three digits. IDs are fixed-width, so a
lexicographic sort gives the true maximum. Never reuse a gap: if `S001` and `S003` exist, the next
ID is `S004`, not `S002`.
